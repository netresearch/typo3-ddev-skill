...
It loads as `typo3-ddev@skills-dir` on the next session. Update with `git -C ~/.claude/skills/typo3-ddev pull` and start a new session; remove it by deleting the directory. This route has no `claude plugin update`.
